...
function init() {
    ...
    TweenLite.set(quad1, {alpha:0});
    TweenLite.set(quad2, {alpha:0, y:+disp/4});
...
function tween1(){
    video1.play();
    video1.addEventListener('ended',tween2,false);
    TweenLite.to(quad1, 0.5, {delay:0.0, alpha:1, x:0, y:0, ease:Power3.easeInOut});
}

//-- FRAME 1 EXIT
function tween2(){
    TweenLite.to(quad1, 0.5, {delay:0.0, alpha:0, ease:Power3.easeInOut});
    TweenLite.to(quad2, 0.5, {delay:0.1, alpha:1, y:0, ease:Power3.easeInOut, onComplete:tween2a});
}

//-- FRAME 2 EXIT
function tween2a(){
   TweenLite.to(quad2, 0.5, {delay:fDelay+0.0, alpha:0, y:disp/4, ease:Power3.easeInOut, onComplete:tween1});
}
